...
It's therefore easy to appreciate why this techniques became known as
`tripleref'
...
All of the above take place with no need for additional `special case' coding.
The timely detection of NULL and the updating (if needed) of the pointer to the
start of the list happen quite automatically.

Use is made, in the routines below,  of the commonplace C shorthand of `while
(*tracer) ' rather than the more general `while (*tracer != NULL) '. But do be
clear that this short-cut relies on boolean values being represented as 0 and 1
and the NULL pointer as 0.   It's possible to envisage exotic C implementations
...
illustrate that they do not need to deliver a formal THING * result for the
purpose of updating the list head (say).

Instead, the content of the incoming formal parameter called `head'
points at the global variable `start' in these examples.  The effect is that the
content of `start' gets updated, automatically and correctly, in all special
cases where "insert at front of list" takes place.  Your reaction to this
behaviour may vary from "That's cool!" to "That's appalling!".
...
result, if this is required for some reason. The `return' statements in these
procedures would then need to be replaced by `return *head' */
